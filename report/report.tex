...
\textbf{Disclaimer: The model dataset as well as this report will be gradually updated as new models and data become available. If possible, please refer to the latest version of this report on ArXiv and the latest version of the data in the Github repository.}
...
The dataset is managed using a versioned Github repository.\footnote{\url{https://github.com/sybila/biodivine-boolean-models/}} Submitting a new model into the collection can be thus performed by anyone using the pull-request functionality (project readme has more detailed instructions about preparing such a pull request). Alternatively, we also accept submissions using issues: Simply create an issue with the bibliographic entry of the model you wish to include and we will try to incorporate it as soon as possible.
...
\paragraph{Keywords} The metadata for each model can contain a set of keywords. In theory, any keyword can be added in order to identify specific classes of models. However, at the moment, we specifically recognise the following keywords:
...
\begin{itemize}
	...
	\item \texttt{repaired}: A model where one or more logical inconsistencies have been resolved (a complete list of possible repairs is given later in this section). The modifications should be listed in model notes.
...
The goal of each repair is to retain original update functions of the model (as these are typically crucial for reproducibility) while adjusting the regulatory graph to match the actual reality.

\section{Models}

In the rest of the report, we give a complete enumeration of all models included in the dataset with basic metadata and associated bibliographic entries.
...
\begin{center}
	...
	\begin{tabular}{ | C{16pt} | C{149pt} | C{25pt} | C{25pt} | C{25pt} | C{36pt} | }
		...
		\hline
		048 & \texttt{GLUCOSE REPRESSION SIGNALING~2009} & 55 & 18 & 97 & \cite{bbm-048} \\
		\hline
